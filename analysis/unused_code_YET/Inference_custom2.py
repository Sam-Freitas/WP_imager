...
imgs_path = (
    r"C:\Users\LabPC2\Documents\GitHub\LightSaver\data\SICKO_testing_dataset_png"
)
...
for i, each_img in enumerate(tqdm.tqdm(imgs)):
    ...
    if len(ann) > 0:
        for j, each_ann in enumerate(ann.data):
            ...
            if (np.asarray(each_ann.sum().cpu()) == np.sum(temp)) and (
                np.sum(temp) > min_area
            ):
                idx.append(j)
        ...
        if ann.shape[0] > 0:
            ...
            props = regionprops_table(
                ...
                properties=["area", "area_convex", "perimeter", "bbox"],
            )
